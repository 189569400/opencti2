...
const graknStateStorage = {
  async load(fn) {
    ...
    const getStatus = `match $status isa MigrationStatus; $status has internal_id $status_id;`;
    ...
    $rel(${RELATION_MIGRATES}_from:$from, ${RELATION_MIGRATES}_to:$to) isa ${RELATION_MIGRATES};`;
